...
const ProductDetails = ({ match }) => {
  ...
  const countdownDate = new Date(product.bidEnd).getTime();
  const now = new Date().getTime();
  const dateEnd = new Date(product.bidEnd);

  if (countdownDate > now) {
    console.log(`Auction Running`);
  } else {
    console.log(`Auction Ended`);
  }

  const refreshTime = (countdownDate - now);
  if (refreshTime > 1) {
  ...
  return (
    <>
      {loading ? (<Loader />) : (
        <>
          <MetaData title={`${product.itemName} Auction`} />
          <div className='product_d'>
                ...
                <div className="row s_product_inner">
                  ...
                  <div className="col-lg-5 offset-lg-1">
                    <div className="s_product_text">
                      ...
                      {(countdownDate > now && userData._id !== sellerDetails._id) ? (<PlaceBid product={product} />)
                        : <h3>! Auction Ended</h3>}
                    </div>
                  </div>
                </div>
                      ...
                      <div className="row">
                        ...
                        <div className="col-lg-6 col-md-6">
                          <div className="icon-box">
                            ...
                            <h3> Created at </h3>
                            <p>{`${created_date}-${created_month}-${created_year} ${created_hours}:${created_minutes}`}</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {winStatus === "Win" && showButton === true ? (
              <StripeCheckout
                stripeKey={process.env.REACT_APP_KEY}
                token={makePayment}
                name="Place bid"
                amount={product.startingBid * 100}
                currency="INR"
              >
